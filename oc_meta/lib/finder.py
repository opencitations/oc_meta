...
from oc_meta.constants import (
    QLEVER_BATCH_SIZE,
    QLEVER_MAX_WORKERS,
    QLEVER_QUERIES_PER_GROUP,
)
...
class ResourceFinder:
    def __init__(
        ...
        self.virtuoso_full_text_search = (
            settings["virtuoso_full_text_search"]
            if settings and "virtuoso_full_text_search" in settings
            else False
        )
    ...
    def _collect_entity_ids(
        self, entity_uri: str, exclude_id_uri: str | None = None
    ) -> list[tuple[str, str]]:
    ...
    def retrieve_br_from_id(
        self, schema: str, value: str
    ) -> List[Tuple[str, str, list]]:
        id_uri = self._find_id_uri(schema, value)
        if not id_uri:
            return []
        metaid_id_list = [
            (id_uri.replace(f"{self.base_iri}/", ""), f"{schema}:{value}")
        ]
        result_list = []
        for entity_uri in self._get_subjects(_P_HAS_IDENTIFIER, id_uri):
            ...
            result_list.append(
                (
                    entity_uri.replace(f"{self.base_iri}/", ""),
                    title,
                    metaid_id_list + other_ids,
                )
            )
        return result_list

    def retrieve_br_from_meta(
        self, metaid: str
    ...
    def retrieve_metaid_from_merged_entity(
        self, metaid_uri: str, prov_config: str
    ) -> str | None:
        """
        ...
        agnostic_meta = AgnosticEntity(
            res=metaid_uri,
            config=prov_config_dict,
        ...
        if meta_history_data:
            ...
            penultimate_snapshot = sorted(
                ...
                key=lambda x: parser.parse(x[1]["generatedAtTime"]).replace(
                    tzinfo=None
                ),
                reverse=True,
            )[1][0]
            ...
            merged_entities = [
                se for se in results if metaid_uri not in se["se"]["value"]
            ]
            if merged_entities:
    ...
    def retrieve_ra_from_id(
        self, schema: str, value: str
    ) -> List[Tuple[str, str, list]]:
        id_uri = self._find_id_uri(schema, value)
        if not id_uri:
            return []
        metaid_id_list: List[Tuple[str, str]] = [
            (id_uri.replace(f"{self.base_iri}/", ""), f"{schema}:{value}")
        ]
        result_list = []
        for entity_uri in self._get_subjects(_P_HAS_IDENTIFIER, id_uri):
    ...
    def retrieve_ra_sequence_from_br_meta(
        self, metaid: str, col_name: str
    ) -> List[Dict[str, tuple]]:
        if col_name == "author":
            role_str = _R_AUTHOR
        ...
        for ar_uri in self._get_objects(metaid_uri, _P_IS_DOC_CONTEXT_FOR):
            ...
            if role_str in roles:
                ...
                next_role = (
                    next_list[0].replace(f"{self.base_iri}/", "") if next_list else ""
                )
        ...
        roles_with_next = set(
            details["next"] for details in dict_ar.values() if details["next"]
        )
        ...
        if len(start_role_candidates) == 0:
            sorted_ars = sorted(
                all_roles, key=lambda ar: get_resource_number(f"{self.base_iri}/{ar}")
            )
        ...
        if len(start_role_candidates) != 1:
            ...
            for start_candidate in start_role_candidates:
                ...
                while current_role and current_role not in visited_roles:
                    ...
                    if current_role in dict_ar:
                        ra_info = self.retrieve_ra_from_meta(
                            dict_ar[current_role]["ra"]
                        )[0:2]
            ...
            chains.sort(
                key=lambda chain: (
                    -len(chain),
                    get_resource_number(f"{self.base_iri}/{list(chain[0].keys())[0]}"),
                )
            )
            try:
            ...
            while current_role and current_role not in visited_roles:
                visited_roles.add(current_role)
                if current_role in dict_ar:
                    ra_info = self.retrieve_ra_from_meta(dict_ar[current_role]["ra"])[
                        0:2
                    ]
                    ra_tuple = ra_info + (dict_ar[current_role]["ra"],)
    ...
    def retrieve_br_info_from_meta(self, metaid: str) -> dict:
        ...
        metaid_uri = (
            f"{self.base_iri}/{metaid}" if self.base_iri not in metaid else metaid
        )
        po = self._get_all_po(metaid_uri)
        res_dict: dict = {
            "pub_date": "",
        ...
        for ar_uri in publisher_ar_uris:
            ...
            for ra_uri in self._get_objects(ar_uri, _P_IS_HELD_BY):
                ...
                for id_uri in ra_po.get(_P_HAS_IDENTIFIER, []):
                    ...
                    literals = id_po.get(_P_HAS_LITERAL_VALUE, [])
                    if schemes and literals:
                        pub_identifiers.append(
                            f"{schemes[0].replace(_DATACITE, '')}:{literals[0]}"
                        )
    ...
    def get_everything_about_res(
        ...
        def process_batch_parallel(subjects, cur_depth, visited_subjects):
            ...
            if len(batch_queries) > 1 and MAX_WORKERS > 1:
                ...
                results = (
                    execute_sparql_queries(endpoint_url=ts_url, queries=batch_queries)
                    if batch_queries
                    else []
                )
            ...
            for result in results:
                for row in result:
                    ...
                    o_datatype = (
                        o_binding.get("datatype", "")
                        if o_binding["type"] in ("literal", "typed-literal")
                        else ""
                    )
            ...
            for batch in batches:
                ...
                if self.virtuoso_full_text_search:
                    ...
                    for identifier in batch:
                        ...
                        identifiers_values.append(
                            f'(<{_DATACITE}{scheme}> "{escaped_literal}"^^<{_XSD_STRING}>)'
                        )
            ...
            if len(batch_queries) > 1 and MAX_WORKERS > 1:
                ...
                with ProcessPoolExecutor(
                    ...
                    for idx, grouped_result in enumerate(
                        executor.map(worker, query_groups)
                    ):
                ...
                results = execute_sparql_queries(
                    endpoint_url=ts_url, queries=batch_queries
                )
                if progress and progress_task is not None:
        ...
        def _build_values_queries(
            issue_vol_tuples, issue_no_vol_tuples, vol_only_tuples
        ):
            queries = []
        ...
        def get_initial_subjects_from_vvis(vvis, progress_task=None):
            ...
            venue_id_to_uris = {}
            if all_venue_ids:
                venue_id_subjects, venue_id_to_uris = (
                    get_initial_subjects_from_identifiers(all_venue_ids)
                )
                    ...
                    for venue_id in venue_ids_tuple:
                        if venue_id in venue_id_to_uris:
                            for venue_uri in venue_id_to_uris[venue_id]:
                                if "/br/" in venue_uri:
                                    venues_to_search.add(
                                        venue_uri.replace(f"{self.base_iri}/", "omid:")
                                    )

                for venue_metaid_to_search in venues_to_search:
                    venue_uri = (
                        f"{self.base_iri}/{venue_metaid_to_search.replace('omid:', '')}"
                    )
                    ...
                    if issue:
                        if volume:
                            issue_vol_tuples.append(
                                (venue_uri, escaped_volume, escaped_issue)
                            )
                        else:
            ...
            vvi_queries = _build_values_queries(
                issue_vol_tuples, issue_no_vol_tuples, vol_only_tuples
            )
            ...
            if len(vvi_queries) > 1 and MAX_WORKERS > 1:
                ...
                results = execute_sparql_queries(
                    endpoint_url=ts_url, queries=vvi_queries
                )
                if progress and progress_task is not None:
        ...
        if identifiers:
            id_subjects, _ = get_initial_subjects_from_identifiers(
                identifiers, progress_task=task_identifiers
            )
        ...
        if vvis:
            initial_subjects.update(
                get_initial_subjects_from_vvis(vvis, progress_task=task_vvis)
            )
        ...
        console.print(
            f"  Max traversal depth reached: {max_depth_reached}/{max_depth}",
            style=style,
        )

    def retrieve_venue_from_local_graph(self, meta_id: str) -> VenueStructure:
        content: VenueStructure = {"issue": {}, "volume": {}}
