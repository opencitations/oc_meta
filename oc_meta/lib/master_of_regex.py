...
RE_ENTITY_URI = re.compile(
    r"^(?P<base>https://w3id\.org/oc/meta)/(?P<short_name>br|ra|ar|re|id)/(?P<supplier_prefix>06[1-9]*0)(?P<entity_number>[1-9]\d*)$"
)
...
alphabets = (
    r"a-zà-öø-ÿ\u0430-я\u0391-ω"  # basic latin, latin-1 supplement, cyrillic, greek
)
vi_pattern = rf"((?:[{alphabets}]*\d[{alphabets}\d]*|[ivxlcdm]+)(?:\s?(?:[{separators}]|and|\()\s?[{alphabets}\d]+\)?)*?)"
numero = r"(?:№|no?(?!v)\.?|n[º°]\.?|n[uú]m(?:[eé]ro)?|number)"
year_pattern = r"(\d{4})"
valid_vi_patterns = [
    vi_pattern,
    rf"[‹<\()]?[{alphabets}]+?([{separators}\s]?[{alphabets}])*[\)›>]?",
    rf"[{alphabets}{separators}\s]+{vi_pattern}",
    rf"[{alphabets}\d\-'/]*\d[{alphabets}\d\-'/]*(,?\s[{alphabets}\d\-'/]+)+",
    rf"\(?s(uppl([eéi]ment(ary|um)?))?\)?\s?(part)?\s?(s?{vi_pattern})?",
    rf"({vi_pattern}_)?({vi_pattern}\s)?[\(_]?supp?(plement|pl)?l?[\s\._]*({vi_pattern}|[{alphabets}])?\)?\.?",
    rf"{vi_pattern}*,?\s?part[\s_]{vi_pattern}(\sof\s{vi_pattern})?(,\sno?\.\s?{vi_pattern})?",
    rf"{vi_pattern}*[_\s]?pt?[_\s\.]{vi_pattern}",
    r"(ed|pt|d)\sinside(d|r)",
    r"p(ublish\s)?a(head\sof\s)?p(rint)?",
    "預刊文章",
    "[\u0621-\u064a]+",
    rf"\[{year_pattern}\]\s(\d\s)?[{alphabets}]+",
    rf"{vi_pattern}\s\[\+CDROM\]",
    rf"{vi_pattern}[{separators}\s]?\({vi_pattern}\)(\s{vi_pattern})?",
    rf"([{alphabets}]+\.)?[{alphabets}]+\.?",
    rf"[{alphabets}]+-\d+",
    rf"[{alphabets}]+(_[{alphabets}]+)+",
    rf"{numero}:?\s?{vi_pattern}(,?\s({year_pattern}|\({vi_pattern}\)))?",
    r"historica\svol\.\s\d+(,\d+(-\d+)?)?",
    r"\d+\(\d+\)\d{2,4}",
    rf"(\[{year_pattern}\]\s)?(\d+\s)?vl?r(\s\([a-z]+\))?",
    rf"\({vi_pattern}\/{vi_pattern}\)\s[{alphabets}]+(-[{alphabets}]+)?",
]
volumes_valid_patterns = [
    r"original\sseries,\svolume\s\d+",
    rf"(vol(ume)?|tome|cilt)\s?[{separators}]?\s?{vi_pattern}",
]
issues_valid_patterns = [
    rf"issue[\.,]?\s{vi_pattern}",
    rf'({vi_pattern}\s)?e?sp?e?(ecial)?[\s_\-\.](issue)?(_number_)?[\s_-]?({vi_pattern})?(["“][{alphabets}\s]+?["”])?',
    rf"ö(zel)?(\ss(ayı)?|\(special\))?(\s?{vi_pattern})?",
    rf"({numero}[{separators}\s]?)?hors[{separators}\s]série[{separators}\s]{vi_pattern}",
    "특별호",
    rf"([{alphabets}]+\s{year_pattern}\s)?\(?(jan(uary)?|feb(ruary)?|mar(ch)?|apr(il)?|may|jun(e)?|jul(y)?|aug(ust)?|sep(tember)?|oct(ober)?|(nov|dec)(ember)?|spring|summer|autumn|winter)(\s{year_pattern})?\)?",
    rf"{vi_pattern},\spart\s{vi_pattern}\sof\s{vi_pattern}",
    rf"sayı[{separators}\s]\s?{vi_pattern}",
    rf"issues?\s{vi_pattern},\s(supplement|part)\s{vi_pattern}",
    rf"issues?\s{vi_pattern}\.?\spp\.\s[a-z\d]+[^a-z\d]+[a-z\d]+",
]
invalid_vi_patterns = {
    rf".*?(?:vol\.?(?:ume)?|tome)(?:[{separators}]?\s?){vi_pattern}[\-&\/_,\.:+;\(\)\[\]|\s]*(?:{numero}|issues?)[{separators}|\s]*(?:sp[eé]cial\s)?{vi_pattern}": "vol_iss",
    rf"{vi_pattern},\s?{numero}\s?{vi_pattern}": "vol_iss",
    rf"tập\s?{vi_pattern},?\s?số\s?{vi_pattern}": "vol_iss",
    rf"issues?\s{vi_pattern}\svol\.?(?:ume)?\s{vi_pattern}(?:.*?{year_pattern}.*?)?": "iss_vol_year",
    rf"{vi_pattern}\s?\({vi_pattern}'{year_pattern}\)": "vol_iss_year",
    rf"cilt[{separators}\s]\s?{vi_pattern}[{separators}\s]sayı[{separators}\s]\s?{vi_pattern}(?:[{separators}\s]\s?temmuz\s{year_pattern})?": "vol_iss_year",
    r"&na;|n\/a|not\savailable": "del",
    r"[\:\-\.`ё/]": "del",
    rf"\${{[{alphabets}]+(\.[{alphabets}]+)?}}": "del",
    rf"[&\/_,:+;\|`'#]\s*{vi_pattern}": "all",
    rf"[\->+]{vi_pattern}": "do_nothing",
    rf"{vi_pattern}[\.+]": "do_nothing",
    rf"{numero}?[{separators}]?\s?{vi_pattern}[&\/_,:;\|`'\(\[\{{]": "all",
    rf"{vi_pattern}\(\)": "all",
    rf"n[�?]+{vi_pattern}": "all",
    rf"{vi_pattern}(?:â\x80[\x92\x93\x94]|�+|â|\?+){vi_pattern}": "sep",
    rf"{vi_pattern}\s?\(first\sserie": "s)",
}
...
RE_VOLUMES_VALID_PATTERNS = [
    re.compile(f"^{pattern}$", re.IGNORECASE) for pattern in volumes_valid_patterns
]
RE_ISSUES_VALID_PATTERNS = [
    re.compile(f"^{pattern}$", re.IGNORECASE) for pattern in issues_valid_patterns
]
