...
# Directories
rdf_base_directory = 'E:/meta_output_current/rdf'

# SPARQL Endpoint URL
endpoint_url = "http://127.0.0.1:19999/blazegraph/sparql"
...
def process_file(args):
    ...
    if file_type == 'json':
        ...
        return process_zip_file(filepath)
    ...
    for root, dirs, files in os.walk(rdf_base_directory):
        ...
        for filename in files:
            if filename.endswith('.json'):
                tasks.append((os.path.join(root, filename), 'json'))
            elif filename.endswith('.zip'):
                tasks.append((os.path.join(root, filename), 'zip'))
