...
def _draw_phase_breakdown(
    ax, collect_ids: float, curation_rest: float, rdf_time: float, storage_time: float
):
    ...
    ax.bar(
        0,
        collect_ids,
        color=CURATION_COLLECT_IDS_COLOR,
        edgecolor="black",
        linewidth=0.5,
        width=0.6,
    )
    ...
    ax.text(
        0,
        curation_total,
        f"{curation_total:.1f}s",
        ha="center",
        va="bottom",
        fontweight="bold",
    )
    ax.text(
        1, rdf_time, f"{rdf_time:.1f}s", ha="center", va="bottom", fontweight="bold"
    )
    ax.text(
        2,
        storage_time,
    ...
    legend_patches = [
        Rectangle(
            ...
            1,
            1,
            facecolor=CURATION_COLLECT_IDS_COLOR,
            edgecolor="black",
            linewidth=0.5,
        ),
        Rectangle(
...
def plot_scalability_analysis(per_size_results: List[Dict[str, Any]], output_path: str):
    ...
    fig.suptitle(
        "Scalability analysis across dataset sizes", fontsize=16, fontweight="bold"
    )
    ...
    for size_data in per_size_results:
        if size_data["statistics"]:
            mean_durations.append(
                size_data["statistics"]["total_duration_seconds"]["mean"]
            )
            mean_throughputs.append(
                size_data["statistics"]["throughput_records_per_sec"]["mean"]
            )
            # Sum all curation sub-phases
            curation_sum = size_data["statistics"][
                "curation__collect_identifiers_duration_seconds"
            ]["mean"] + sum(
                size_data["statistics"][f"{p}_duration_seconds"]["mean"]
                for p in CURATION_REST_PHASES
            )
            phase_data["curation"].append(curation_sum)
            phase_data["rdf_creation"].append(
                size_data["statistics"]["rdf_creation_duration_seconds"]["mean"]
            )
            phase_data["storage_upload"].append(
                size_data["statistics"]
                .get("storage_duration_seconds", {})
                .get("mean", 0)
            )
        else:
            ...
            phase_data["rdf_creation"].append(
                get_phase_duration_by_name(run, "rdf_creation")
            )
            phase_data["storage_upload"].append(get_storage_total(run))

    axes[0, 0].plot(
        sizes, mean_durations, marker="o", linewidth=2, markersize=8, color="#2E86AB"
    )
    apply_plot_style(
        axes[0, 0],
        "Total duration vs dataset size",
    ...
    apply_plot_style(
        axes[1, 0],
        "Phase breakdown by dataset size",
        "Dataset size (records)",
        "Duration (s)",
    )
    ...
    if len(sizes) > 1 and len(per_size_results[0]["runs"]) > 1:
        for size_data in per_size_results:
            ...
            axes[1, 1].plot(
                [size] * len(durations), durations, "o", alpha=0.6, markersize=6
            )

        apply_plot_style(
            axes[1, 1],
            "Duration distribution across sizes",
            "Dataset size (records)",
            "Duration (s)",
        )
        axes[1, 1].plot(
        ...
        bars = axes[1, 1].bar(
            sizes, mean_durations, color="#2E86AB", edgecolor="black", linewidth=1.5
        )
        format_bar_labels(axes[1, 1], bars, mean_durations, "s")
        apply_plot_style(
            axes[1, 1],
            "Total duration by dataset size",
            "Dataset size (records)",
            "Duration (s)",
        )
...
def plot_benchmark_results(
    all_runs: List[Dict[str, Any]], stats: Dict[str, Dict[str, Any]], output_path: str
):
    ...
    axes[0, 0].plot(
        run_numbers, total_times, marker="o", linewidth=2, markersize=8, color="#2E86AB"
    )
    axes[0, 0].axhline(
        y=stats["total_duration_seconds"]["mean"],
        color="#A23B72",
        linestyle="--",
        label="Mean",
        linewidth=1.5,
    ...
    apply_plot_style(
        axes[0, 0], "Total duration per run", "Run number", "Total duration (s)"
    )
    axes[0, 0].legend()

    # Phase breakdown
    collect_ids_mean = stats["curation__collect_identifiers_duration_seconds"]["mean"]
    curation_rest_mean = sum(
        stats[f"{p}_duration_seconds"]["mean"] for p in CURATION_REST_PHASES
    )
    rdf_mean = stats["rdf_creation_duration_seconds"]["mean"]
    storage_mean = stats.get("storage_duration_seconds", {}).get("mean", 0)

    _draw_phase_breakdown(
        axes[0, 1], collect_ids_mean, curation_rest_mean, rdf_mean, storage_mean
    )
    apply_plot_style(
        axes[0, 1],
        "Average phase duration breakdown",
        ylabel="Duration (s)",
        grid=False,
    )
    ...
    bp = axes[1, 0].boxplot(
        [curation_times, rdf_times, upload_times],
    ...
    apply_plot_style(
        axes[1, 0],
        "Phase duration distribution (box plot)",
        ylabel="Duration (s)",
        grid=False,
    )
    ...
    legend_elements = [
        Rectangle(
            (0, 0), 1, 1, facecolor="gray", alpha=0.5, label="Box: 50% of data (Q1-Q3)"
        ),
    ...
    axes[1, 1].bar(
        run_numbers, throughputs, color=STORAGE_COLOR, edgecolor="black", linewidth=1.5
    )
    axes[1, 1].axhline(
        y=stats["throughput_records_per_sec"]["mean"],
        color="#A23B72",
        linestyle="--",
        label="Mean",
        linewidth=1.5,
    ...
    apply_plot_style(
        axes[1, 1], "Throughput per run", "Run number", "Throughput (records/sec)"
    )
    axes[1, 1].legend()
...
def plot_single_run_results(run: Dict[str, Any], output_path: str):
    ...
    apply_plot_style(
        axes[0], "Phase duration breakdown", ylabel="Duration (s)", grid=False
    )
    ...
    apply_plot_style(
        axes[1], "Processing throughput", ylabel="Records per second", grid=False
    )
...
def _get_peak_memory_from_report(report: Dict[str, Any]) -> float:
    ...
    peaks = [
        p["peak_memory_mb"] for p in report.get("phases", []) if p["peak_memory_mb"]
    ]
...
def plot_incremental_progress(
    ...
    curation_phase_times = {
        phase_name: [
            _get_phase_duration_from_report(r["report"], phase_name)
            for r in all_reports
        ]
        for phase_name, _, _ in CURATION_PHASES
    }
    rdf_times = [
        _get_phase_duration_from_report(r["report"], "rdf_creation")
        for r in all_reports
    ]
    storage_times = (
    ...
    curation_totals = [
        sum(curation_phase_times[p][i] for p, _, _ in CURATION_PHASES)
        for i in range(len(filenames))
    ]
    total_times = [
        c + r + s for c, r, s in zip(curation_totals, rdf_times, storage_times)
    ]
    ...
    ax1.set_title(
        f"Processing time by phase ({len(filenames)} files processed)",
        fontsize=14,
        fontweight="bold",
    )
    ax1.set_xticks(x)
    ax1.set_xticklabels(
        [f[:20] for f in filenames], rotation=45, ha="right", fontsize=8
    )
    ...
    ax2.axhline(
        ...
        linestyle="--",
        linewidth=2,
        label=f"Mean ({mean_throughput:.1f} rec/s)",
    )
    ...
    ax3.plot(
        x, peak_memories, marker="s", linewidth=2, markersize=6, color=MEMORY_COLOR
    )
    ax3.axhline(
        y=mean_memory,
        color="#A23B72",
