...
def generate_unique_output_filename(zip_path, json_filename):
    hash_digest = hashlib.sha256(zip_path.encode()).hexdigest()[:10]  # Prendiamo solo i primi 10 caratteri per brevità
    unique_filename = f"{hash_digest}_{json_filename}.ttl"
    return unique_filename

def process_zip_file(args):
    zip_path, output_folder, queue = args
    with zipfile.ZipFile(zip_path, 'r') as zip_ref:
        for file_info in zip_ref.infolist():
            if file_info.filename.endswith('.json'):
                with zip_ref.open(file_info) as json_file:
                    json_file_data = json_file.read()
                    process_jsonld_data(json_file_data, file_info.filename, zip_path, output_folder)
    queue.put(1)  # Signal completion of one ZIP file

def process_jsonld_data(json_data, original_filename, zip_path, output_folder):
    g = ConjunctiveGraph()
    g.parse(source=json_data, format='json-ld')
    unique_output_filename = generate_unique_output_filename(zip_path, original_filename)
    output_file_path = os.path.join(output_folder, unique_output_filename)
    g.serialize(format='ttl', destination=output_file_path, encoding='utf-8')
...
def main():
    ...
    with Manager() as manager:
        ...
        with multiprocessing.Pool(multiprocessing.cpu_count()) as pool:
            pool.map_async(process_zip_file, [(zip_path, args.output_folder, queue) for zip_path in zip_files])
            pool.close()
