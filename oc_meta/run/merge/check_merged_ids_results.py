        ...
        with zipfile.ZipFile(prov_file_path, "r") as zip_ref:
            ...
            for i, snapshot in enumerate(snapshots):
                ...
                if i < len(snapshots) - 1 or not is_surviving:
                    ...
                    is_surviving
                    and g.value(snapshot, PROV.invalidatedAtTime) is not None
                ):
                    tqdm.write(
                ...
                if i == 0:  # First snapshot
                    ...
                    if is_merge:
                        if len(derived_from) < 2:
                            tqdm.write(
                                f"Error in provenance file {prov_file_path}: Merge snapshot {snapshot} should be derived from more than one snapshot"
                            )
                    else:
                        if len(derived_from) != 1:
                            tqdm.write(
                                f"Error in provenance file {prov_file_path}: Non-merge snapshot {snapshot} should have exactly one prov:wasDerivedFrom relation, but has {len(derived_from)}"
                            )
...
def main():
    ...
    with ctx.Pool(processes=multiprocessing.cpu_count()) as pool:
        list(
            tqdm(
                pool.imap(process_entity, all_tasks),
                total=len(all_tasks),
