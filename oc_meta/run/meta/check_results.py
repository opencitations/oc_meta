...
_STAT_FIELDS = (
    ...
    "rows_with_ids",
    "total_identifiers",
    "omid_schema_identifiers",
    "identifiers_skipped_invalid",
    "identifiers_skipped_unverifiable",
    "identifiers_with_omids",
    "identifiers_without_omids",
    "identifiers_with_omid_mismatch",
    "data_graphs_found",
    "data_graphs_missing",
    "prov_graphs_found",
    "prov_graphs_missing",
    "omids_with_provenance",
    "omids_without_provenance",
)
...
def check_provenance_existence(
    omids: List[str],
    prov_endpoint_url: str,
...
def check_omids_existence(
    ...
    for i in range(0, len(identifiers), QLEVER_BATCH_SIZE):
        ...
        for identifier in batch:
            escaped_value = (
                identifier["value"].replace("\\", "\\\\").replace('"', '\\"')
            )
            values_entries.append(
                f'("{escaped_value}"^^xsd:string datacite:{identifier["schema"]})'
            )
...
def _check_zip_file(args: tuple) -> tuple[str, dict[str, tuple[bool, bool]]]:
    ...
    for omid in omids:
        ...
        results[omid] = (
            omid_bytes in raw_data,
            (omid + '/prov/"').encode() in raw_prov,
        )
...
def process_csv_file(
    args: tuple, workers: int = QLEVER_MAX_WORKERS, progress=None, task_id=None
) -> FileResult:
    (
        csv_file,
        endpoint_url,
    ...
    with open(csv_file, "rb") as f:
        raw = f.read()
    df = pl.read_csv(
        raw.replace(b"\0", b""), columns=_ID_COLUMNS, infer_schema_length=0
    )
    df = df.with_columns(
        [
            pl.col(c)
    ...
    if progress:
        phase1_task = progress.add_task(
            "  Phase 1/5: Extracting identifiers", total=result.total_rows, detail=""
        )
    ...
    for row_idx in range(result.total_rows):
        ...
        for col in _ID_COLUMNS:
            ...
            for group in groups:
                n_tokens = (
                    (1 if group["omid_uri"] else 0)
                    + len(group["recognized"])
                    + len(group["unverifiable"])
                )
                if n_tokens == 0:
                    continue
                ...
                for schema, value in group["recognized"]:
                    ...
                    if id_key not in recognized_id_set:
                        ...
                        recognized_ids.append(
                            {"schema": norm_schema, "value": norm_value}
                        )
    ...
    if progress:
        phase2_task = progress.add_task(
            "  Phase 2/5: Querying DB", total=len(recognized_ids), detail=""
        )
    ...
    identifier_cache = check_omids_existence(
        recognized_ids, endpoint_url, workers=workers, progress_callback=on_id_batch
    )
    ...
    if progress:
        phase3_task = progress.add_task(
            "  Phase 3/5: Mapping OMIDs", total=len(recognized_id_set), detail=""
        )
    ...
    for id_key in recognized_id_set:
        ...
        if id_key in identifier_cache:
            ...
            result.id_key_locations[id_key] = [
                {"file": csv_basename, "row": r, "column": c} for r, c in occurrences
            ]
    ...
    for entity_uri in all_entity_uris:
        ...
        zip_path = find_rdf_file(
            entity_uri, rdf_dir, dir_split_number, items_per_file, zip_output_rdf
        )
    ...
    phase4_task = None
    if progress:
        phase4_task = progress.add_task(
            "  Phase 4/5: Checking RDF files", total=total_rdf_files, detail=""
        )
    ...
    phase5_task = None
    if progress:
        phase5_task = progress.add_task(
            "  Phase 5/5: Checking provenance", total=total_omids, detail=""
        )
...
def main():
    ...
    parser.add_argument(
        "--workers",
        type=int,
        default=QLEVER_MAX_WORKERS,
        help=f"Max parallel SPARQL workers (default: {QLEVER_MAX_WORKERS})",
    )
    ...
    process_args = [
        (
            ...
            prov_endpoint_url,
            output_rdf_dir,
            config["dir_split_number"],
            config["items_per_file"],
            config["zip_output_rdf"],
            base_iri,
        )
        for f in csv_files
    ]
    ...
    with Progress(
        ...
        task = progress.add_task(
            "Processing CSV files", total=len(csv_files), detail=""
        )
        ...
        for idx, proc_args in enumerate(process_args):
            ...
            file_result = process_csv_file(
                proc_args, workers=args.workers, progress=progress, task_id=task
            )
