...
def store_in_file(cur_g: Dataset, cur_file_path: str, zip_output: bool) -> None:
    ...
    if zip_output:
        with ZipFile(
            cur_file_path, mode="w", compression=ZIP_DEFLATED, allowZip64=True
        ) as zip_file:
            json_str = orjson.dumps(cur_json_ld).decode("utf-8")
            zip_file.writestr(
                os.path.basename(cur_file_path.replace(".zip", ".json")), json_str
            )
    else:
...
def load_graph(file_path: str, cur_format: str = "json-ld"):
    ...
    if file_path.endswith(".zip"):
        with ZipFile(
            file=file_path, mode="r", compression=ZIP_DEFLATED, allowZip64=True
        ) as archive:
                ...
                with archive.open(zf_name) as f:
                    if cur_format == "json-ld":
                        ...
                        for json_ld_resource in json_ld_file:
                            loaded_graph.parse(
                                data=orjson.dumps(json_ld_resource).decode("utf-8"),
                                format=cur_format,
                            )
                    else:
...
def process_graph(
    context, graph_identifier, output_root, base_iri, file_limit, item_limit, zip_output
):
    ...
    for triple in context:
        triples += len(triple)
        entity_uri = triple[0]
        _, cur_file_path = find_paths(
            entity_uri, output_root, base_iri, "_", file_limit, item_limit, True
        )
...
def merge_files(output_root, base_file_name, file_extension, zip_output):
    ...
    files_to_merge = [
        ...
        if f.startswith(base_file_name) and f.endswith(file_extension)
    ]
...
def merge_files_in_directory(directory, zip_output, stop_file):
    ...
    files = [
        ...
        if f.endswith(".zip" if zip_output else ".json")
    ]
    ...
    for base_file_name, files_to_merge in file_groups.items():
        ...
        final_file_path = os.path.join(
            directory, f"{base_file_name}" + (".zip" if zip_output else ".json")
        )
...
def process_file_content(
    file_path, output_root, base_iri, file_limit, item_limit, zip_output, rdf_format
):
...
def mark_file_as_processed(file_path, cache_file):
    if cache_file:
        ...
        logging.debug(
            f"No cache file specified. Skipping marking {file_path} as processed."
        )
...
def main():
    ...
    parser.add_argument(
        "input_folder", type=str, help="Input folder containing gzipped input files"
    )
    parser.add_argument(
        "output_root", type=str, help="Root folder for output OC Meta RDF files"
    )
    parser.add_argument(
        "--base_iri",
        type=str,
        default="https://w3id.org/oc/meta/",
        help="The base URI of entities on Meta",
    )
    parser.add_argument(
        "--file_limit", type=int, default=10000, help="Number of files per folder"
    )
    parser.add_argument(
        "--item_limit", type=int, default=1000, help="Number of items per file"
    )
    parser.add_argument(
        "-v",
        "--zip_output",
    ...
    parser.add_argument(
        ...
        type=int,
        default=1000,
        help="Number of files to process before merging",
    )
    parser.add_argument(
        "--cache_file",
        type=str,
        default=None,
        help="File to store processed file names (optional)",
    )
    parser.add_argument(
        "--stop_file",
        type=str,
        default="./.stop",
        help="File to signal process termination",
    )
    ...
    chunks = [
        files_to_process[i : i + args.chunk_size]
        for i in range(0, len(files_to_process), args.chunk_size)
    ]
