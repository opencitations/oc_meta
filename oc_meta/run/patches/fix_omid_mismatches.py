...
def execute_merges(
    ...
    rows: list[MergeRow] = [
        {
            "surviving_entity": case.surviving_entity,
            "merged_entities": list(case.duplicate_entities),
        }
    ...
    console.print(
        ...
        "files, then rerun with --cleanup to remove the duplicate identifiers.[/bold]"
    )
...
def execute_cleanup(
    ...
    entries = [e for e in report["merge"] if e["duplicate_id_entities"]]
    if not entries:
    ...
    with create_progress() as progress:
        task = progress.add_task("Removing duplicate identifiers", total=len(entries))
        for entry in entries:
            ...
            entry_key = (
                f"{entry['surviving_entity']}|{','.join(entry['duplicate_entities'])}"
            )
            if entry_key in completed:
                progress.advance(task)
                continue

            for id_ent in entry["duplicate_id_entities"]:
                editor.delete(entry["surviving_entity"], DATACITE_HAS_ID, id_ent)
...
def main() -> None:  # pragma: no cover
    ...
    parser.add_argument("--check-results", help="Path to check_results.json")
    ...
    mode.add_argument(
        "--cleanup",
        action="store_true",
        help="Remove the duplicate identifiers listed in the report "
    ...
    if args.no_dry_run:
        if summary["merge"]:
            ...
            execute_merges(cases, args.config, args.resp_agent)
    else:
        console.print(
