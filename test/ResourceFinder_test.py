...
class TestResourceFinder:
    ...
    def test_retrieve_venue_from_local_graph(self, finder):
        ...
        # Venue
        finder.add_triple(venue_uri, GraphEntity.iri_has_sequence_identifier, 'Journal Test')
...
class TestVVIQueryIsolation:
    ...
    def test_vvi_queries_only_search_correct_venues(self, vvi_endpoint):
        ...
        assert venue_a_uri in volume_10_parents, \
        ...
        assert venue_b_uri in volume_20_parents, \
...
class TestFinderGetSubgraph:
    ...
    def test_get_subgraph_reconstructs_typed_literals(self):
        ...
        finder.add_triple(br, GraphEntity.iri_has_publication_date, date, o_datatype="http://www.w3.org/2001/XMLSchema#date")
        g = finder.graph.subgraph(br)
        assert g is not None
        triples = set(g)
        assert (br, GraphEntity.iri_title, RDFTerm("literal", title, _XSD_STRING)) in triples
        assert (br, GraphEntity.iri_has_publication_date, RDFTerm("literal", date, "http://www.w3.org/2001/XMLSchema#date")) in triples
        assert len(triples) == 2
    ...
    def test_get_subgraph_reconstructs_plain_literals(self):
        ...
        assert g is not None
        triples = set(g)
        assert (br, GraphEntity.iri_title, RDFTerm("literal", value, _XSD_STRING)) in triples
...
class TestFinderRetrieveRaFromIdMultipleIds:
    ...
    def finder_with_ra_multiple_ids(self):
        ...
        finder.add_triple(id1_uri, GraphEntity.iri_uses_identifier_scheme, GraphEntity.DATACITE["orcid"])
        finder.add_triple(id1_uri, GraphEntity.iri_has_literal_value, "0000-0001-1234-5678")

        finder.add_triple(ra_uri, GraphEntity.iri_has_identifier, id2_uri)
        finder.add_triple(id2_uri, GraphEntity.iri_uses_identifier_scheme, GraphEntity.DATACITE["viaf"])
