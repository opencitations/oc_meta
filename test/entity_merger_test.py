...
class TestEntityMerger(unittest.TestCase):
    ...
    def setup_test_data(self):
        ...
        g_set = GraphSet("https://w3id.org/oc/meta/", supplier_prefix="060", custom_counter_handler=self.counter_handler)
        
        # Create first author entity with specific ID
        author1 = g_set.add_ra(resp_agent="https://orcid.org/0000-0002-8420-0696", 
                            res=URIRef("https://w3id.org/oc/meta/ra/0601"))
        author1.has_name("John Smith")
        
        # Create ORCID identifier for author1 with specific ID
        orcid_id = g_set.add_id(resp_agent="https://orcid.org/0000-0002-8420-0696",
        ...
        author2 = g_set.add_ra(resp_agent="https://orcid.org/0000-0002-8420-0696",
                            res=URIRef("https://w3id.org/oc/meta/ra/0602"))
        author2.has_name("J. Smith")
        
        # Create VIAF identifier for author2 with specific ID
        viaf_id = g_set.add_id(resp_agent="https://orcid.org/0000-0002-8420-0696",
        ...
        pub = g_set.add_br(resp_agent="https://orcid.org/0000-0002-8420-0696",
        ...
        role1 = g_set.add_ar(resp_agent="https://orcid.org/0000-0002-8420-0696",
        ...
        role2 = g_set.add_ar(resp_agent="https://orcid.org/0000-0002-8420-0696",
                            res=URIRef("https://w3id.org/oc/meta/ar/0602"))
        role2.create_author()
        role2.is_held_by(author2)
        pub.has_contributor(role2)
        
        prov = ProvSet(g_set, "https://w3id.org/oc/meta/", wanted_label=False, custom_counter_handler=self.counter_handler)
        prov.generate_provenance()

        rdf_output = os.path.join(OUTPUT, 'rdf') + os.sep

        res_storer = Storer(abstract_set=g_set, dir_split=10000, 
                            n_file_item=1000, output_format='json-ld', 
                            zip_output=False)
        prov_storer = Storer(abstract_set=prov, dir_split=10000, 
                            n_file_item=1000, output_format='json-ld',
                            zip_output=False)
        ...
        res_storer.upload_all(triplestore_url=SERVER, base_dir=rdf_output, batch_size=10, save_queries=False)
    ...
    def check_sparql_query_content(self, query: str, expected_triples: dict):
        ...
        delete_match = re.search(r'DELETE DATA \{ GRAPH.*?\{(.*?)\}.*?\}', query, re.DOTALL)
        insert_match = re.search(r'INSERT DATA \{ GRAPH.*?\{(.*?)\}.*?\}', query, re.DOTALL)
        ...
        if 'delete' in expected_triples:
            for triple in expected_triples['delete']:
                self.assertIn(triple, delete_section.strip(), 
        ...
        if 'insert' in expected_triples:
            for triple in expected_triples['insert']:
                self.assertIn(triple, insert_section.strip(), 
    ...
    def test_get_entity_type(self):
        ...
        for url, expected in test_cases:
            with self.subTest(url=url):
                self.assertEqual(
                    EntityMerger.get_entity_type(url),
                    expected
                )
    ...
    def test_process_folder_with_worker_limit(self):
        ...
        large_data = [{
            ...
            'Done': 'False'
        } for i in range(15000)]  # Create more than 10000 rows
        ...
        self.assertEqual(large_file_data[0]['Done'], 'False')  # Large file should be skipped

        small_file_data = EntityMerger.read_csv(os.path.join(csv_folder, 'merge_test.csv'))
        self.assertEqual(small_file_data[0]['Done'], 'True')  # Small file should be processed
    ...
    def test_merge_authors_with_real_data(self):
        ...
        self.assertTrue(os.path.exists(os.path.join(rdf_path, 'rdf', 'ra', '060', '10000', '1000')))
        self.assertTrue(os.path.exists(os.path.join(rdf_path, 'rdf', 'ra', '060', '10000', '1000', 'prov')))
            ...
            for graph in data:
                for entity in graph.get('@graph', []):
                    if entity['@id'] == 'https://w3id.org/oc/meta/ra/0601':
                        ...
                        # Check name
                        self.assertEqual(entity['http://xmlns.com/foaf/0.1/name'][0]['@value'], 'J. Smith')
        ...
        with open(ar_file) as f:
            ...
            for graph in data:
                for entity in graph.get('@graph', []):
                    ...
                    self.assertEqual(agent, 'https://w3id.org/oc/meta/ra/0601', 
                                'All roles should point to surviving entity')
        
        # Check provenance
        prov_file = os.path.join(rdf_path, 'rdf', 'ra', '060', '10000', '1000', 'prov', 'se.json')
        ...
        with open(prov_file) as f:
            ...
            for graph in data:
                for entity in graph.get('@graph', []):
                    if '/prov/se/' in entity['@id'] and 'merge' in entity.get('http://purl.org/dc/terms/description', [{}])[0].get('@value', '').lower():
                        found_merge_prov = True

                        # Check provenance fields
                        self.assertIn('http://www.w3.org/ns/prov#generatedAtTime', entity)
                        self.assertIn('http://www.w3.org/ns/prov#wasAttributedTo', entity)
                        self.assertIn('https://w3id.org/oc/ontology/hasUpdateQuery', entity)
                        
                        # Get actual query and normalize both expected and actual
                        actual_query = entity['https://w3id.org/oc/ontology/hasUpdateQuery'][0]['@value']
    ...
    def test_merge_multiple_entities(self):
        ...
        author3 = g_set.add_ra(resp_agent="https://orcid.org/0000-0002-8420-0696",
                            res=URIRef("https://w3id.org/oc/meta/ra/0603"))
        author3.has_name("John A. Smith")
        
        author4 = g_set.add_ra(resp_agent="https://orcid.org/0000-0002-8420-0696",
        ...
        researcher_id = g_set.add_id(resp_agent="https://orcid.org/0000-0002-8420-0696",
        ...
        role3 = g_set.add_ar(resp_agent="https://orcid.org/0000-0002-8420-0696",
                        res=URIRef("https://w3id.org/oc/meta/ar/0603"))
        role3.create_author()
        role3.is_held_by(author3)
        pub2.has_contributor(role3)
        
        role4 = g_set.add_ar(resp_agent="https://orcid.org/0000-0002-8420-0696",
            ...
            for graph in data:
                for entity in graph.get('@graph', []):
                    if entity['@id'] == 'https://w3id.org/oc/meta/ra/0601':
                        ...
                        self.assertEqual(entity['http://xmlns.com/foaf/0.1/name'][0]['@value'], 'J A Smith')
                    
                    # Check merged entities no longer exist
                    self.assertNotIn(entity['@id'], [
        ...
        # 3. Check provenance
        prov_file = os.path.join(rdf_path, 'ra', '060', '10000', '1000', 'prov', 'se.json')
        with open(prov_file) as f:
            data = json.load(f)
            ...
            for graph in data:
                if graph['@id'] == 'https://w3id.org/oc/meta/ra/0601/prov/':
                    for entity in graph.get('@graph', []):
                        ...
                        if 'created' not in entity.get('http://purl.org/dc/terms/description', [{}])[0].get('@value', '').lower():
                            surviving_snapshots.append(entity)
            
            # Should have 2 merge snapshots (one partial, one final)
            self.assertEqual(len(surviving_snapshots), 2, 
                            "Should have exactly 2 merge snapshots")
            
            # Verify partial merge (0601 with 0602)
            partial_merge = next(s for s in surviving_snapshots 
            ...
            partial_query = partial_merge['https://w3id.org/oc/ontology/hasUpdateQuery'][0]['@value']
            expected_partial = {
                'delete': [
                    '<https://w3id.org/oc/meta/ra/0601> <http://xmlns.com/foaf/0.1/name> "John Smith"'
                ],
            ...
            final_query = final_merge['https://w3id.org/oc/ontology/hasUpdateQuery'][0]['@value']
            expected_final = {
                'delete': [
                    '<https://w3id.org/oc/meta/ra/0601> <http://xmlns.com/foaf/0.1/name> "John Smith"'
                ],
            ...
            for merged_id in merged_ids:
                ...
                for graph in data:
                    if graph['@id'] == f'https://w3id.org/oc/meta/ra/{merged_id}/prov/':
                        for entity in graph.get('@graph', []):
                            if 'deleted' in entity.get('http://purl.org/dc/terms/description', [{}])[0].get('@value', '').lower():
                                merged_snapshots.append(entity)
                
                self.assertGreater(len(merged_snapshots), 0,
                                f"No deletion snapshot found for ra/{merged_id}")
                
                # Verify deletion queries
                for snapshot in merged_snapshots:
                    self.assertIn('https://w3id.org/oc/ontology/hasUpdateQuery', snapshot)
                    delete_query = snapshot['https://w3id.org/oc/ontology/hasUpdateQuery'][0]['@value']
                    self.assertIn(f'<https://w3id.org/oc/meta/ra/{merged_id}>', delete_query)
    ...
    def test_merge_with_conflicting_data(self):
        ...
        author5 = g_set.add_ra(resp_agent="https://orcid.org/0000-0002-8420-0696",
        ...
        author6 = g_set.add_ra(resp_agent="https://orcid.org/0000-0002-8420-0696",
        ...
        same_orcid = g_set.add_id(resp_agent="https://orcid.org/0000-0002-8420-0696",
                            res=URIRef("https://w3id.org/oc/meta/id/0605"))
        same_orcid.create_orcid("0000-0001-9999-9999")
        author5.has_identifier(same_orcid)
        
        same_orcid2 = g_set.add_id(resp_agent="https://orcid.org/0000-0002-8420-0696",
            ...
            for graph in data:
                for entity in graph.get('@graph', []):
                    if entity['@id'] == 'https://w3id.org/oc/meta/ra/0605':
                        ...
                        identifiers = {id_obj['@id'] for id_obj in entity.get('http://purl.org/spar/datacite/hasIdentifier', [])}
                        self.assertEqual(len(identifiers), 1)
                        self.assertEqual(identifiers, {'https://w3id.org/oc/meta/id/0605'})
                        
                        # Check name was preserved
                        self.assertEqual(entity['http://xmlns.com/foaf/0.1/name'][0]['@value'], 'Johnny Smith')
                        self.assertEqual(entity['http://xmlns.com/foaf/0.1/givenName'][0]['@value'], 'Johnny')
                        self.assertEqual(entity['http://xmlns.com/foaf/0.1/familyName'][0]['@value'], 'Smith')
                    
                    # Check merged entity does not exist in output
                    self.assertNotEqual(entity['@id'], 'https://w3id.org/oc/meta/ra/0606')
        ...
        with open(prov_file) as f:
            ...
            self.assertIsNotNone(delete_snapshot, "No deletion snapshot found for merged entity")
            
            # Verify deletion query
            delete_query = delete_snapshot['https://w3id.org/oc/ontology/hasUpdateQuery'][0]['@value']
            self.assertIn('DELETE DATA', delete_query)
    ...
    def test_merge_bibliographic_resources(self):
        ...
        issue = g_set.add_br(resp_agent="https://orcid.org/0000-0002-8420-0696",
        ...
        re1 = g_set.add_re(resp_agent="https://orcid.org/0000-0002-8420-0696",
        ...
        doi_id = g_set.add_id(resp_agent="https://orcid.org/0000-0002-8420-0696",
                        res=URIRef("https://w3id.org/oc/meta/id/0603"))
        ...
        volume = g_set.add_br(resp_agent="https://orcid.org/0000-0002-8420-0696",
        ...
        re2 = g_set.add_re(resp_agent="https://orcid.org/0000-0002-8420-0696",
        ...
        isbn_id = g_set.add_id(resp_agent="https://orcid.org/0000-0002-8420-0696",
                            res=URIRef("https://w3id.org/oc/meta/id/0604"))
            ...
            for graph in data:
                for entity in graph.get('@graph', []):
                    if entity['@id'] == 'https://w3id.org/oc/meta/br/0603':
                        ...
                        self.assertEqual(
                            entity['http://purl.org/spar/fabio/hasSubtitle'][0]['@value'],
                        ...
                        parts = {part['@id'] for part in entity['http://purl.org/vocab/frbr/core#partOf']}
                        self.assertEqual(len(parts), 1)
                        self.assertIn('https://w3id.org/oc/meta/br/0606', parts)  # Volume
                        
                        # Check formats (resource embodiments)
                        formats = {fmt['@id'] for fmt in entity['http://purl.org/vocab/frbr/core#embodiment']}
                    ...
                    # Check issue metadata
                        ...
                        self.assertIn('http://purl.org/spar/fabio/JournalIssue', 
                                    entity['@type'])
                        self.assertEqual(
                            entity['http://purl.org/spar/fabio/hasSequenceIdentifier'][0]['@value'],
                            '4'
                        )
                    
                    # Check volume metadata
                    elif entity['@id'] == 'https://w3id.org/oc/meta/br/0606':
                        self.assertIn('http://purl.org/spar/fabio/JournalVolume',
                                    entity['@type'])
                        self.assertEqual(
                            entity['http://purl.org/spar/fabio/hasSequenceIdentifier'][0]['@value'],
                            '15'
                    ...
                    # Check merged entity no longer exists
                    self.assertNotEqual(entity['@id'], 'https://w3id.org/oc/meta/br/0604')
        ...
        with open(re_file) as f:
            ...
            for graph in data:
                for entity in graph.get('@graph', []):
                    if entity['@id'] in ['https://w3id.org/oc/meta/re/0603', 'https://w3id.org/oc/meta/re/0604']:
                        res_embodiments[entity['@id']] = {
                            'start': entity['http://prismstandard.org/namespaces/basic/2.0/startingPage'][0]['@value'],
                            'end': entity['http://prismstandard.org/namespaces/basic/2.0/endingPage'][0]['@value']
                        }
            
            self.assertEqual(len(res_embodiments), 2)
            self.assertEqual(res_embodiments['https://w3id.org/oc/meta/re/0603']['start'], '1')
            self.assertEqual(res_embodiments['https://w3id.org/oc/meta/re/0603']['end'], '20')
            self.assertEqual(res_embodiments['https://w3id.org/oc/meta/re/0604']['start'], '100')
            self.assertEqual(res_embodiments['https://w3id.org/oc/meta/re/0604']['end'], '120')
        ...
        # 4. Check provenance
        prov_file = os.path.join(rdf_path, 'br', '060', '10000', '1000', 'prov', 'se.json')
        with open(prov_file) as f:
            data = json.load(f)
            ...
            for graph in data:
                if graph['@id'] == 'https://w3id.org/oc/meta/br/0604/prov/':
                    ...
                    self.check_sparql_query_content(delete_query, expected_delete_triples)
                    ...
                    for graph in data:
                        ...
                        if graph['@id'] == 'https://w3id.org/oc/meta/br/0606/prov/':
                            ...
                            for entity in graph.get('@graph', []):
                                if 'created' in entity.get('http://purl.org/dc/terms/description', [{}])[0].get('@value', '').lower():
                                    found_volume_creation = True
                                    self.assertIn('http://www.w3.org/ns/prov#generatedAtTime', entity)
                                    self.assertIn('http://www.w3.org/ns/prov#wasAttributedTo', entity)
                            self.assertTrue(found_volume_creation, "No creation provenance found for volume")
                        ...
                        if graph['@id'] == 'https://w3id.org/oc/meta/re/0604/prov/':
                            ...
                            for entity in graph.get('@graph', []):
                                if 'created' in entity.get('http://purl.org/dc/terms/description', [{}])[0].get('@value', '').lower():
                                    ...
                                    self.assertIn('http://www.w3.org/ns/prov#generatedAtTime', entity)
                                    self.assertIn('http://www.w3.org/ns/prov#wasAttributedTo', entity)
                            self.assertTrue(found_re_creation, "No creation provenance found for resource embodiment")
                    ...
                    for graph in data:
                        if graph['@id'] == 'https://w3id.org/oc/meta/br/0603/prov/':
                            for entity in graph.get('@graph', []):
                                if 'merge' in entity.get('http://purl.org/dc/terms/description', [{}])[0].get('@value', '').lower():
                                    timestamp = entity['http://www.w3.org/ns/prov#generatedAtTime'][0]['@value']
                                    merge_timestamps.append(timestamp)
                    
                    # Check timestamps are in correct order
                    self.assertEqual(len(merge_timestamps), 1, "Should have exactly one merge operation")
                    ...
                    with open(br_file) as f:
                        ...
                        for graph in data:
                            for entity in graph.get('@graph', []):
                                if entity['@id'] == 'https://w3id.org/oc/meta/br/0606':  # Volume
                                    volume_found = True
                                    self.assertIn('http://purl.org/spar/fabio/JournalVolume', entity['@type'])
                                    
                        self.assertTrue(volume_found, "Volume should still exist after merge")
                    ...
                    with open(re_file) as f:
                        ...
                        for graph in data:
                            for entity in graph.get('@graph', []):
                                if entity['@id'] == 'https://w3id.org/oc/meta/re/0604':  # RE from merged entity
                                    re_found = True
                                    self.assertEqual(
                                        entity['http://prismstandard.org/namespaces/basic/2.0/startingPage'][0]['@value'],
                                        '100'
                                    )
    ...
    #     """
    #     Create test data with different distributions of entities across rows.
        
    #     Args:
    #         total_entities: Total number of entities to create
    #         distribution_type: How to distribute entities across rows
    #             - 'single_row': All entities in one row
    #             - 'equal_rows': Entities distributed equally across multiple rows
    #             - 'mixed': Mix of single rows and multiple rows
                
    #     Returns:
    #         List of dictionaries containing merge data for CSV
    #     """
    #     g_set = GraphSet("https://w3id.org/oc/meta/", supplier_prefix="060",
    #                     custom_counter_handler=self.counter_handler)
    ...
    #     # Create merge data with different distributions
    #     merge_data = []
        
    #     if distribution_type == 'single_row':
    #         # All entities in one row
    #         surviving_entity = f"https://w3id.org/oc/meta/ra/0601"
    #         merged_entities = [f"https://w3id.org/oc/meta/ra/060{i}" 
    ...
    #     elif distribution_type == 'equal_rows':
    #         # Distribute entities equally across rows (2 entities per row)
    #         for i in range(1, total_entities - 1, 2):
    #             merge_data.append({
    #                 'surviving_entity': f"https://w3id.org/oc/meta/ra/060{i}",
    #                 'merged_entities': f"https://w3id.org/oc/meta/ra/060{i+1}",
    #                 'Done': 'False'
    #             })
                
    #     elif distribution_type == 'mixed':
    #         # Mix of single rows and multiple rows
    #         # First 40% in one row
    #         split_point = int(total_entities * 0.4)
    #         surviving_entity = f"https://w3id.org/oc/meta/ra/0601"
    #         merged_entities = [f"https://w3id.org/oc/meta/ra/060{i}" 
    #                         for i in range(2, split_point + 1)]
    #         merge_data.append({
    #             'surviving_entity': surviving_entity,
    #             'merged_entities': '; '.join(merged_entities),
    #             'Done': 'False'
    ...
    #         # Remaining 60% distributed in pairs
    #         for i in range(split_point + 1, total_entities - 1, 2):
    #             merge_data.append({
    #                 'surviving_entity': f"https://w3id.org/oc/meta/ra/060{i}",
    #                 'merged_entities': f"https://w3id.org/oc/meta/ra/060{i+1}",
    #                 'Done': 'False'
    ...
    # def measure_merge_performance_with_distributions(self, total_entities: int, 
    #                                             batch_sizes: List[int],
    #                                             distributions: List[str], 
    #                                             runs: int = 3) -> Dict[str, Dict[int, List[float]]]:
    #     """
    #     Measure merge performance across different batch sizes and entity distributions.
    ...
    #     Returns:
    #         Nested dict mapping distribution types and batch sizes to execution times
    #     """
    #     results = {dist: {batch_size: [] for batch_size in batch_sizes} 
    ...
    #                 # Setup fresh test data and get merge data
    #                 merge_data = self.setup_performance_test_data_with_distributions(
    #                     total_entities, distribution)
    ...
    #                 results[distribution][batch_size].append(execution_time)
                    
    #     return results

    # def test_merge_performance_analysis(self):
    #     """Test performance characteristics of entity merging with different batch sizes and distributions"""
    #     # Test parameters
    #     total_entities = 1000  # Total number of entities to merge
    #     batch_sizes = [1, 5, 10, 20, 50]  # Different batch sizes to test
    #     distributions = ['single_row', 'equal_rows', 'mixed']  # Different distribution types
    #     runs = 3  # Number of runs for averaging
        
    #     # Run performance measurements
    #     results = self.measure_merge_performance_with_distributions(
    #         total_entities, batch_sizes, distributions, runs)
        
    #     # Calculate statistics for each distribution
    #     stats = {dist: {} for dist in distributions}
    #     for dist in distributions:
    #         for batch_size, times in results[dist].items():
    #             avg_time = statistics.mean(times)
    #             std_dev = statistics.stdev(times) if len(times) > 1 else 0
    #             stats[dist][batch_size] = {
    #                 'avg_time': avg_time,
    #                 'std_dev': std_dev,
    #                 'raw_times': times
    #             }
    ...
    #         base_time = stats[dist][1]['avg_time']  # baseline (batch_size=1)
    #         print("\nEfficiency Metrics:")
    #         for size in sorted(batch_sizes):
    #             efficiency = base_time / stats[dist][size]['avg_time']
    #             print(f"Batch Size {size:2d}: {efficiency:.2f}x speedup")
            
    #         print("\nExecution Times (seconds):")
    #         for size in sorted(batch_sizes):
    #             print(f"\nBatch Size: {size}")
    #             print(f"  Average: {stats[dist][size]['avg_time']:.3f}s")
    #             print(f"  Std Dev: {stats[dist][size]['std_dev']:.3f}s")
    #             print(f"  Raw times: {', '.join(f'{t:.3f}' for t in stats[dist][size]['raw_times'])}")
            
    #         optimal_size = min(stats[dist].keys(), key=lambda x: stats[dist][x]['avg_time'])
