...
class TestGetSubjectsOfClass:
    def test_returns_subject_values(self) -> None:
        with patch(
            "oc_meta.run.migration.extract_subset.execute_sparql",
            return_value=_bindings([{"s": {"value": ENTITY_A}}]),
        ):
...
class TestParseObject:
    ...
    def test_typed_literal(self) -> None:
        ...
        assert parse_object(result) == rdflib.Literal(
...
class TestGetTriplesForEntities:
    def test_with_graphs(self) -> None:
        ...
        with patch(
            "oc_meta.run.migration.extract_subset.execute_sparql", return_value=response
        ):
            quads = get_triples_for_entities("http://x", [ENTITY_A], use_graphs=True)
        assert quads == [
            (
                rdflib.URIRef(ENTITY_A),
                rdflib.URIRef("http://ex.org/p"),
                rdflib.URIRef(ENTITY_B),
                rdflib.URIRef(GRAPH_URI),
            )
        ]
...
class TestExtractSubset:
    def test_class_uri_with_graphs_and_compress(self, tmp_path: Path) -> None:
        ...
        with patch(
            "oc_meta.run.migration.extract_subset.execute_sparql", side_effect=responses
        ):
            count, result_path = extract_subset(
                "http://x", 10, output, True, class_uri=CLASS_URI
            )
        assert count == 1
    ...
    def test_compress_with_gz_extension(self, tmp_path: Path) -> None:
        ...
        with patch(
            ...
            _count, result_path = extract_subset(
                "http://x", 10, output, True, class_uri=CLASS_URI
            )
        assert result_path == output
    ...
    def test_recurse_follows_uri_objects(self, tmp_path: Path) -> None:
        ...
        with patch(
            ...
            count, _ = extract_subset(
                "http://x", 10, output, False, class_uri=CLASS_URI, use_graphs=False
            )
        assert count == 2
