    ...
    def test_optimize_groups_data_loss_bug(self):
        ...
        for i in range(35):
            grouped_data[f"group{i}"] = pd.DataFrame(
                [{"surviving_entity": f"e{i}", "merged_entities": f"e{i + 100}"}]
            )
        ...
        assert total_rows_input == total_rows_output, (
...
class TestGroupEntities:
    ...
    def test_group_entities_creates_groups(self, mock_get_related):
        ...
        df = pd.DataFrame(
            [
                ...
                {
                    "surviving_entity": "https://example.org/e3",
    ...
    def test_group_entities_no_double_iteration(self, mock_get_related):
        ...
        assert df_mock.iterrows.call_count == 1, (
    ...
    def test_complete_workflow(self, mock_get_related):
        ...
        with open(csv_path, "w", newline="") as f:
            writer = csv.DictWriter(
                f, fieldnames=["surviving_entity", "merged_entities"]
            )
            writer.writeheader()
            for i in range(5):
                writer.writerow(
