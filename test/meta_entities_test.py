...
class TestOCMetaStatistics:
    ...
    def test_count_role_entities(self):
        ...
        expected = {"pro:author": 4, "pro:publisher": 2, "pro:editor": 1}
...
"""
    ...
    def test_run_selected_analyses_br_only(self):
        with OCMetaStatistics(SERVER) as stats:
            results = stats.run_selected_analyses(
                analyze_br=True, analyze_ar=False, analyze_venues=False
            )
    ...
    def test_run_selected_analyses_ar_only(self):
        with OCMetaStatistics(SERVER) as stats:
            results = stats.run_selected_analyses(
                analyze_br=False, analyze_ar=True, analyze_venues=False
            )

        expected = {"roles": {"pro:author": 4, "pro:publisher": 2, "pro:editor": 1}}
        assert results == expected

    def test_run_selected_analyses_venues_without_csv_path(self):
        with OCMetaStatistics(SERVER) as stats:
            results = stats.run_selected_analyses(
                analyze_br=False, analyze_ar=False, analyze_venues=True
            )
    ...
    def test_run_all_analyses(self):
        ...
        expected = {
            "fabio_expressions": 3,
            "roles": {"pro:author": 4, "pro:publisher": 2, "pro:editor": 1},
            "venues": 2,
        }
        assert results == expected
